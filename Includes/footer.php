
<<<<<<< HEAD
  <body>
      <div>
        <footer class="bottomArea">
        </footer>
      </div>
=======
<!--  <body> why another body tag -->
<!--      <div> don't just add tags without checking...  -->
        </div>
        <div class="bottomArea">
            <h3> Unit 1-2 North Point Business Centre, Horton Road, London SWL 8EQ </h3>
            <a href="rss.php" title="RSS"><i class="fa fa-rss fa-2x" aria-hidden="true"></i> RSS</a>
        </div>


>>>>>>> origin/master
      <script src="JavaScript/mobNav.js"></script>
      <script src="JavaScript/searchFunction.js"></script>
      <div id="footer">
      <h5> Contact us... </h5> <h6>Unit 1-2 North Point Business Centre, Horton Road, London SWL 8EQ </h6>
    </div>
    </body>
</html>
